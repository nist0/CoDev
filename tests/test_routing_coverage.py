...
def test_strict_zero_threshold_on_real_matrix(tmp_path: Path) -> None:
    ...
    # The real matrix has 85 gaps; threshold=0 means 0 gaps allowed, so must exit 1.
    assert rc == 1, "strict+threshold=0 must exit 1 when gaps exist"


def test_strict_threshold_85_passes_on_real_matrix() -> None:
    """The CI gate uses --strict --threshold 85 (current gap count on the real matrix).
    This must pass on the real matrix, proving the gate is correctly calibrated."""
    rc = main(strict=True, threshold=85, json_out=None)
    assert rc == 0, "strict+threshold=85 must exit 0 on the current matrix (85 gaps)"
